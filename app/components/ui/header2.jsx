  ...
  return (
    <header className="border-b border-neutral-600">
      <div className="flex items-center justify-between px-6 py-2 bg-deep_red max-[62rem]:px-4">
        <div className="text-left font-semibold space-x-5 text-heritage_gold text-body_s max-xl:text-body_xs max-62rem::hidden">
      ...
      <div className="max-[62rem]:py-7 max-[62rem]:px-2">
        <nav className="bg-white border-b border-neutral-600 max-[62rem]:border-hidden relative ">
          <div className="flex justify-between px-6 py-5 max-[62rem]:flex-col items-start w-full max-[62rem]:py-0 max-[62rem]:px-2">
            <ul className="flex max-[62rem]:flex-col max-[62rem]:w-full">
              {navLeft.map((item, index) => (
                ...
                >
                  ...
                  {item.hasDropdown && item.dropdownItems.length > 0 && (
                    <div className="h-[100vh] absolute left-0 top-[61px] bg-white border-neutral-400 p-6 border-r max-w-[27rem] w-full opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-10">
                      <ul>
                            ...
                            {dropdownItem.hasCourses &&
                              dropdownItem.courses.length > 0 && (
                                <div className="h-[100vh] absolute left-full top-[-1.5rem] bg-neutral-50 border-l border-neutral-900 p-4 w-[calc(100vw-26.5rem)] opacity-0 invisible group-hover/submenu:opacity-100 group-hover/submenu:visible transition-all duration-300 z-10">
                                  {/* <div className="h-[27.75rem] absolute left-full top-[-1.5rem] bg-neutral-50 border-l border-neutral-900 p-4 w-[calc(100vw-26.5rem)] opacity-0 invisible group-hover/submenu:opacity-100 group-hover/submenu:visible transition-all duration-300 z-10"> */}
                                  <ul>
            ...
            <div className="flex justify-end space-x-4 max-[62rem]:flex-col max-[62rem]:pb-4">
              {navRight.map((item) => (
                <Link
                  key={item.name}
                  to={item.link}
